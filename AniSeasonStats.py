...
def save_to_excel_with_formatting_and_autofit(all_stats, filename):
    rows = []
    for stat in all_stats:
        row = {
            "season": stat["Season"],
            "year": stat["Year"],
            "anime_count": stat["Anime Count"],
            "mean_score": stat["Mean Score"],
            "weighted_mean": stat["Weighted Mean"]
        }
    ...
    apply_custom_formatting(ws, "E", top=2, bottom=2)  # Combined Weighted Mean

    apply_autofit(ws, ["C", "D", "E"])  # Autofit for specific columns
...
if __name__ == "__main__":
    ...
    if not username:
        ...
        save_to_excel_with_formatting_and_autofit(stats, "anime_season_stats")
